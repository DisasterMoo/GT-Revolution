...
public abstract class IndustrialMachine extends RecipeMapMultiblockController
{
    ...
    private IndustrialType type;
    private int parallel;
    private float euCostMult, durationMult;

    public IndustrialMachine(ResourceLocation metaTileEntityId, RecipeMap<?> recipes, IndustrialType type)
    {
    ...
    protected class IndustrialParallelLogic extends MultiblockRecipeLogic
    {
        ...
        IndustrialParallelLogic(RecipeMapMultiblockController tileEntity)
        {
            super(tileEntity);
        }
        ...
        protected Recipe findRecipe(long maxVoltage, IItemHandlerModifiable inputs, IMultipleTankHandler fluidInputs)
        {
            ...
            for (int index = 0; index < inputs.getSlots(); index++)
            {
                ...
                if (inputIngredient != null && (maxItemsLimit - currentItemsEngaged) >= inputIngredient.getCount())
                {
                    ...
                    {
                        for (Recipe.ChanceEntry entry : matchingRecipe.getChancedOutputs())
                        {
                            ItemStack it = entry.getItemStack().copy();
                            it.setCount(it.getCount() * overclockAmount);
                            recipeOutputs.add(it);
                        }
                    }
    ...
    protected class IndustrialEfficientLogic extends MultiblockRecipeLogic
    {
        ...
        IndustrialEfficientLogic(RecipeMapMultiblockController tileEntity)
        {
            super(tileEntity);
        }
