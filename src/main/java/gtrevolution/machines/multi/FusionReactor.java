 ...
 */
public class FusionReactor extends RecipeMapMultiblockController
{
    private final int tier;
    private EnergyContainerList inputEnergyContainers;

    public FusionReactor(ResourceLocation metaTileEntityId, int tier)
    {
    ...
    {
        return new FusionReactor(metaTileEntityId, tier);
    }
    ...
    {
        return GRTextures.FUSION_TEXTURE;
    }
        ...
        if (!getWorld().isRemote)
        {
            ...
            if (this.recipeMapWorkable.isWorkingEnabled())
            {
                ...
                if (previousRecipe != recipe)
                {
                    if (recipe != null)
                    {
                        long euToStart = (Integer) recipe.getProperty("eu_to_start");
    ...
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline)
    {
        ...
        GRTextures.FUSION_REACTOR_OVERLAY.render(renderState, translation, pipeline, this.getFrontFacing(), this.recipeMapWorkable.isActive());
    }
